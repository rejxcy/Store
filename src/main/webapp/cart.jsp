...
<body>
	...
	<hr width="100%" />
	<div class="text3" align="center">購物車中的商品</div>
	<br>
	...
	<table width="100%" border="0" align="center" class="threeboder">
		<tr bgcolor="#A5D3FF">
			...
			<td width="15%" align="center" class="theader">金額</td>
		</tr>
		<c:forEach var="row" items="${cart}">
			<tr>
				...
				<input name="quantity_${row.goodsid}" type="text" value="${row.quantity}"
						onblur="calc(${row.goodsid}, this)" /></td>
				<td align="center" class="trow">NT :<span id="price_${row.goodsid}">${row.price}</span></td>
				<td align="center" class="trow">NT :<span id="subtotal_${row.goodsid}">${row.price * row.quantity}</span></td>
			</tr>
		</c:forEach>

		<tr>
			<td height="50" colspan="5" align="right">合計 :<span id="total">${total}</span>&nbsp;&nbsp;
			</td>
		</tr>
	</table>
	<br>
